...
import { stepMachine, setMode } from './circuit'
...
describe('element math', () => {
  ...
  it('integrator derivative matches summer', () => {
...
describe('vehicle suspension', () => {
  it('fits within amplifier budget', () => {
    const m = loadVehicleSuspension('firm')
    expect(countAmplifiers(m.nodes)).toBeLessThanOrEqual(36)
  })
  ...
  it('exposes analog X/Y figure voltages', () => {
    ...
    const yb =
      m.lastEval.voltages[portKey({ nodeId: 'sum_yb', port: 'out' })] ?? NaN
  ...
  it('free response decays with firm damping', () => {
    let m = loadVehicleSuspension('firm')
    m = {
      ...m,
      nodes: m.nodes.map((n) =>
...
describe('algebraic evaluation', () => {
  it('detects algebraic loops', () => {
    ...
    const result = evaluateAlgebraic(nodes, cables, {}, 'operate')
    expect(result.warning).toMatch(/algebraic loop/i)
  })
})
